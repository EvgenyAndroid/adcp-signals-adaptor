...
Passes the official AdCP **storyboard** suite (`@adcp/sdk@13.0.2`, compliance line 3.1.20): **57 / 57 scenarios**, 0 failed steps, 76 steps skipped by applicability (2026-09-08, auto-written to `src/constants/complianceState.ts` by `npm run compliance`). The AAO hosted grader runs the same line twice daily and lists the agent **verified** (66/68 scenarios; 9 of 35 storyboards, the other 21 not applicable to a signals-only agent) — feed: `https://agenticadvertising.org/api/registry/agents/https%3A%2F%2Fadcp.signal-stack.io%2Fmcp/compliance`. The 7/7 `@adcp/client@5.25.1` figure that used to sit here was the legacy scenario suite, retired 2026-09-07.
...
`comply_test_controller` response-schema check. Compliance-runner behavior
